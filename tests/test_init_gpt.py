...
class TestInitGpt(unittest.TestCase):
    ...
    @patch('src.lib.init_gpt.communicate_with_gpt')
    @patch('src.lib.init_gpt.manage_conversation_history')
    def test_init_gpt_with_empty_user_input(self, mock_manage_conversation_history, mock_communicate_with_gpt, mock_get_user_input, mock_read_initial_prompt, mock_load_env, mock_openai):
        # Setup mock behavior
        mock_read_initial_prompt.return_value = "Initial prompt"
        # Simulate user input scenarios: valid input, empty input, and then exit
        mock_get_user_input.side_effect = ["User input", "", None]
        mock_communicate_with_gpt.return_value = "GPT response"
        ...
        # Check if communication with GPT is happening only once
        mock_communicate_with_gpt.assert_called_once_with(
            mock_openai(), 
            init_gpt.CHATGPT_MODEL, 
            [
                ...
                {'role': 'user', 'content': 'Initial prompt'},
                {'role': 'user', 'content': 'User input'},
                {'role': 'assistant', 'content': 'GPT response'}
            ]
        )
